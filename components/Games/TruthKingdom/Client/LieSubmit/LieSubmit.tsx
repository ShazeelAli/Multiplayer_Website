...
export default function FibbageClientLieSubmit({ roomState, clientWebsocket }: { roomState: RoomState, clientWebsocket: clientWebsocket }) {
    ...
    var display = <div><h1 className={SharedStyles.basic} style={{ textAlign: "center", fontSize: "30px", border: "0px" }}>Submit Your Lie</h1>
        <input type="text" name="Room_Code" className={SharedStyles.basic} style={{ backgroundColor: "white" }} placeholder="Type Lie Here" onChange={handleInputMessage}></input>
        <button disabled={!canSend} className={SharedStyles.button} onClick={sendMessage}> Send </button></div>

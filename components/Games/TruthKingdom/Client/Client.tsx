...
export default function Client({
  ...
  useEffect(() => {
    ...
    socket.on("TK_lies_submission_finished", (sender, senderRTCID, lieList) => {
      var newAnswerList: Map<string, Player> = lieList;
      setAnswersList(newAnswerList);
      setGameState(GameStates.LIE_CHOOSE);
    });
  }, []);

  return <div className={SharedStyles.background_client}>{display}</div>;
}

...
export default function FibbageHostViewScore({
  roomState,
  clientWebsocket,
  playersChose,
  lieList,
  ...
  const endViewing = () => {
    setClosed(true);
    setTimeout(() => {
      socket.emit("relay", "TK_next_round");
    }, 3000);
  };
  ...
  sortedPlayers.forEach((player, index) => {
    ...
    var score = (
      <div
        key={player.name}
        className={[styles.score, sharedStyles.basic].join(" ")}
      >
        <>
          {player.name} : {lie}{" "}
        </>
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={[styles.score_container].join(" ")}>{ScoreDisplay}</div>

      <div className={[styles.footer, sharedStyles.basic].join(" ")}>
        <button className={sharedStyles.button} onClick={endViewing}>
          Next Round
        </button>
      </div>
    </div>

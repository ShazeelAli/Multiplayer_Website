...
import BGM from "../../BGM";
export default function FibbageHostLieChosen({
  roomState,
  clientWebsocket,
  playersChose,
  lieList,
  ...
  playersChose: Map<string, Player[]>;
  lieList: Map<string, Player>;
  currentQuestion: string[];
}) {
  const socket = clientWebsocket.socket;
  const [popPlay, popData] = useSound("/TruthKingdom/pop.mp3", {
    interrupt: true,
    volume: 1,
  });
  const [whooshPlay, whooshData] = useSound("/TruthKingdom/whoosh.mp3", {
    interrupt: true,
    volume: 1,
  });
  ...
  const onContinue = () => {
    ...
    setTimeout(() => {
      socket.emit("relay", "TK_end_lie_viewing");
    }, 4000);
  };
    ...
    playersChose.forEach((playerList, lie) => {
      if (lieList.get(lie) == null) {
        playerList.forEach((player) => {
          socket.emit("updateScore", player, 100);
        });
      } else {
  ...
  useEffect(() => {
    lieList.forEach((Player, lie) => {
      ...
      if (playersChose.has(lie)) {
        ...
        playerList.forEach((player) => {
          ...
          var playerNameDisplay = (
            <span
              className={[styles.player_chose, sharedStyles.basic].join(" ")}
              style={{ animationDelay: smallAnimDelay + "s" }}
              key={player.name}
              ref={playerChoseLieNameRef}
            >
              {player.name}
            </span>
          );
      ...
      lieDisplay.push(
        <div
          className={[styles.lie, sharedStyles.basic].join(" ")}
          style={{ animationDelay: bigAnimDelay + "s" }}
          key={lie}
        >
          {lie}
    ...
    lieDisplay.push(
      ...
      >
        {truth}
        <div>{playersChoseThisLieDisplay}</div>
    ...
    >
      ...
      <div className={styles.container} ref={containerRef}>
        ...
        <div className={[styles.footer, sharedStyles.basic].join(" ")}>
          <button className={sharedStyles.button} onClick={onContinue}>
            Continue
          </button>
        </div>
      </div>

...
export default function HostTutorial({
  clientWebsocket,
}: {
  clientWebsocket: clientWebsocket;
}) {
  const [introPlay, introData] = useSound(
    "/TruthKingdom/VoiceLines/Intro.mp3",
    { interrupt: true, volume: 1 }
  );
  const [introSkipPlay, introSkipData] = useSound(
    "/TruthKingdom/VoiceLines/SkipIntro.mp3",
    { interrupt: true, volume: 1 }
  );
  ...
  return (
    <div>
      ...
      <button className={sharedStyles.button} onClick={skip}>
        SKIP
      </button>
    </div>
  );

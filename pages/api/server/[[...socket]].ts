...
import { ServerToClientEvents, ClientToServerEvents, P2PEvents } from "common";
...
const SocketHandler = (req, res) => {
  ...
  if (res.socket.server.io) {
    ...
    io.on("connection", (socket) => {
      ...
      socket.on("relay", (eventName, ...args) => {
      ...
      socket.on("relayTarget", async (targetID, eventName, ...args) => {
        var player: Player = clients.get(socket);
        var room: RoomState = rooms.get(player.room_code);
